#!/bin/bash
# SchoolDash installer — fetched and run straight from the website:
#
...
#
# SchoolDash is not signed with a paid Apple Developer ID, so anything a
# *browser* downloads gets tagged com.apple.quarantine and Gatekeeper refuses
# to open it — "Apple could not verify SchoolDash is free of malware". There is
...
BASE="${SCHOOLDASH_INSTALL_BASE:-https://jomonkey22.github.io/schooldash-install}"
...
APPS="${SCHOOLDASH_APPS:-$HOME/Applications}"
APP="$APPS/SchoolDash.app"
DRY_RUN="${SCHOOLDASH_INSTALL_DRY_RUN:-}"

say()  { printf '%s\n' "$*"; }
step() { printf '\n==> %s\n' "$*"; }
fail() { printf '\nInstall stopped: %s\n' "$*" >&2; exit 1; }

say "SchoolDash installer"
say "===================="

[ "$(uname -s)" = "Darwin" ] || fail "SchoolDash is a Mac app, and this is not a Mac."

# ---------------------------------------------------------------------------
step "Downloading SchoolDash"
...
tar -tzf "$WORK/$ARCHIVE" | grep -q '^SchoolDash.app/' \
  || fail "the archive does not contain SchoolDash.app. Please report this."
...
# "SchoolDash.app/Contents/MacOS/schooldash", the launcher's own path — but the
...
# "SchoolDash.app/..." pattern. [MEASURED] 2026-09-09, minutes after the fix
# above: the unscoped version matched every copy on the Mac, so
# test_the_installer_installs — which points SCHOOLDASH_APPS at a temp folder
...
running() {
  ...
  pgrep -f "$APP/Contents/MacOS/schooldash" >/dev/null 2>&1
}
if running; then
  say "    closing the running copy first"
  pkill -f "$APP/Contents/Resources/app/run.py" 2>/dev/null || true
  pkill -f "$APP/Contents/MacOS/schooldash" 2>/dev/null || true
  sleep 1
...
if mv "$WORK/SchoolDash.app" "$APP"; then
  rm -rf "$APP.replacing"
else
...
if (apps) {
  ...
  for (var i = 0; i < apps.count; i++) {
    ...
    // Every SchoolDash tile goes, wherever it pointed. One is then added
    // back, so this also cleans up duplicates from earlier installs.
    if (String(url).indexOf('SchoolDash.app') === -1) kept.addObject(tile);
  }
...
cat <<DONE
    ...
    Done. SchoolDash $NEW_VERSION is installed in $APPS.
DONE

cat <<'DONE'

    SchoolDash is starting now. The first launch downloads what it needs —
    ...
    press Connect; SchoolDash signs in to Canvas and Veracross for you.

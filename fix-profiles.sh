...
DATA="$HOME/Library/Application Support/SchoolDash/data"
SITE="${SCHOOLDASH_INSTALL_BASE:-https://jomonkey22.github.io/schooldash-install}"

say()  { printf '%s\n' "$*"; }
fail() { printf '\n  %s\n\n' "$*" >&2; exit 1; }

say ''
say 'SchoolDash — rebuild the class list for your own courses'
say '-------------------------------------------------------'

[ -d "$DATA" ] || fail "SchoolDash has never run on this Mac ($DATA is missing).
...
say ''
say '  updating SchoolDash and rebuilding from your own courses…'
say ''
...
say ''
say '  Done. SchoolDash should now list only your classes.'

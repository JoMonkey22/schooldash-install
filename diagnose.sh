#!/bin/bash
# Why won't Silly Bus AI open on this Mac?
#
...
APP="$HOME/Applications/Silly Bus AI.app"
DATA="$HOME/Library/Application Support/Silly Bus AI"

line() { printf '%s\n' "------------------------------------------------------------"; }
row()  { printf '  %-34s %s\n' "$1" "$2"; }

printf '\nSilly Bus AI — what is wrong on this Mac\n'
line
...
if [ ! -d "$APP" ]; then
  row "Silly Bus AI.app" "NOT INSTALLED at ~/Applications"
...
row "Silly Bus AI.app" "installed"
row "  launcher is executable" "$([ -x "$APP/Contents/MacOS/sillybus" ] && echo yes || echo 'NO — this alone stops it opening')"
...
probe="$(SILLYBUS_LAUNCHER_DRY_RUN=1 \
         SILLYBUS_DATA_HOME="$(mktemp -d)" \
         "$APP/Contents/MacOS/sillybus" 2>&1)"
...
if xattr "$APP" 2>/dev/null | grep -q quarantine; then
  ...
  printf '      "%s/Contents/MacOS/sillybus"\n' "$APP"
fi

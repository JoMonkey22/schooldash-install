...
Rather not use Terminal? Download **SchoolDash-Installer.dmg** from this repo
and read the READ ME FIRST inside; that route shows a Gatekeeper warning and
explains the one-time override.

This repo holds only the installer. Version 0.18.20.0.

- [`install.sh`](install.sh) — read it before you run it
- [`schooldash-chrome.zip`](schooldash-chrome.zip) — the optional Chrome
  ...
  Canvas and Veracross pages to SchoolDash, instead of SchoolDash signing in
  itself.
